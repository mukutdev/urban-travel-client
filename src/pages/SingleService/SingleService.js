    ...
    const reviewErInfo = {
        ...
        .then(data => {
          if(data.acknowledged){
            setRefresh(true)
            swal("Congratzz!", "Your site has been added", "success");
            form.reset()
            console.log(data);
        }
        })
    ...
    <div className="review-section my-14 bg-white mx-auto container rounded-lg p-6 ">
      <div>
        ...
        <div className="reviews-area w-2/3 mx-auto my-7 flex flex-col gap-4">
                 {
                    tripReview ?  tripReview.map(singleReview => <ReviewCard key={singleReview._id} review={singleReview}></ReviewCard>) : <p className="text-center my-5 font-bold text-xl">No Reviews Found</p>  
                 } 
          </div>
      </div>
