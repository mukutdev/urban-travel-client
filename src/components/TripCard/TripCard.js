...
const TripCard = ({trips}) => {
    ...
    const {_id ,tripName , location , tripDetails , tripLength,Price,capacity,thumbnail,Rating} = trips;
        ...
        <div className='shadow-lg'>
            <div className=' relative w-full'>
                ...
                <div className='bg-sky-500 w-full px-4 py-3 flex justify-between gap-2 list-none absolute bottom-0 '>
                 ...
                 <li className='flex gap-1 text-white items-center'><FiUsers/> <span>People: {capacity}</span></li>
                 <li className='flex gap-1 text-white items-center'><ImLocation /> <span>{location}</span></li>
               </div>
               <p className='absolute top-4 px-3 text right-4 bg-orange-500 py-2 flex items-center text-white'>
                <strong className='flex items-center text-xl'><FaDollarSign/>{Price}</strong> / per person</p>
            </div>
                ...
                <div className='px-7 pt-3 pb-7'>
                        ...
                        <p className='flex gap-1 items-center py-3'>
                            ...
                            {
                             [...Array(Rating).keys()].map((k , index) => k===0 ? <AiTwotoneStar key={index} className='text-amber-400 text-lg' />: <AiTwotoneStar key={index} className='text-amber-400 text-lg' /> )
                        }
                        </p>
